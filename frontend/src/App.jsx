...
function App() {
  ...
  const fetchProjects = async () => {
    try {
      const res = await fetch('https://heavy-eagles-grab.loca.lt/api/projects');
  ...
  const fetchTasks = async (projectId) => {
    try {
      const res = await fetch(`https://heavy-eagles-grab.loca.lt/api/tasks/project/${projectId}`);
      const data = await res.json();
      setTasks(data);
    } catch (err) {
  ...
  const handleAuthSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`https://heavy-eagles-grab.loca.lt${endpoint}`, {
        method: 'POST',
      ...
      if (isLogin && data.user) {
        ...
        const resProj = await fetch('https://heavy-eagles-grab.loca.lt/api/projects');
  ...
  const handleCreateTask = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch('https://heavy-eagles-grab.loca.lt/api/tasks', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          title: newTaskTitle,
  ...
  const handleUpdateStatus = async (taskId, currentStatus) => {
    ...
    try {
      await fetch(`https://heavy-eagles-grab.loca.lt/api/tasks/${taskId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
